        ...
        <div class="row px-4 py-2 gap-4">
            <div class="col">
                ...
                <div id="update-shop-form">
                    @csrf
                    <input type="hidden" name="_method" value="PUT"> <!-- Method spoofing for PUT -->
                        ...
                        <div class="align-items-center gap-3" id="managerRow1" style="display:flex;">
                            <select class="form-control px-3 py-2" id="managerName1" name="managers[]">
                                @foreach($managers as $manager)
                                    <option value="{{ $manager->id }}" {{ in_array($manager->id, old('managers', [])) ? 'selected' : '' }}>{{ $manager->first_name }} {{ $manager->last_name }}</option>
                                @endforeach
                            </select>
                ...
                <div class="d-flex gap-2 justify-content-end">
                    <button class="btn btn-link text-primary fw-medium fs-4" type="button" onclick="cancel()">Cancel</button>
                    <button class="btn btn-primary p-2 px-4 fs-4" type="button" onclick="submitShopUpdate()">Save Changes</button>
                </div>
            </div>
        </div>
    </div>
