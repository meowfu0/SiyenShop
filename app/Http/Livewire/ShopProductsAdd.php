...
class ShopProductsAddController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $sizes = $validated['sizes']; // Array of sizes from form
        $variantStocks = $validated['variantStocks']; // Array of variant stock quantities

        // After the product is inserted, use the product ID for the variants
        foreach ($sizes as $index => $size) {
            DB::table('product_variants')->insert([
                'product_id' => $productId, // Now using the correct product ID
                'size' => $size,
